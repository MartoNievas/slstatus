...
#define MODULE(name) "$HOME/dev/suckless-btw/slstatus/scripts/" name
...
static const struct arg args[] = {
    ...
    {run_command, "\x06 ^b#16161e^^c#ff9e64^ %s ",
     "pactl get-default-sink | grep -Ei 'hyperx|usb' >/dev/null && echo '󰋋' "
     "|| echo '󰓃'"},
    ...
    {run_command, "\x04 |  %s°C ",
     "sensors k10temp-pci-00c3 2>/dev/null | awk '/Tctl|Tdie/{print $2}' | sed "
     "'s/+//;s/°C//' | head -n1"},
    {ram_perc, "\x03 |  %s%% ", NULL},
    {disk_perc, "\x02 |  %s%% ^d^ ", "/"},
